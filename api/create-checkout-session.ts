...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!['Standard', 'Premium', 'Max'].includes(plan)) return res.status(400).json({ error: 'Invalid plan' });
    const sb = admin();
    ...
    if (!priceId) return res.status(500).json({ error: `Stripe price id is missing for ${plan}. Add STRIPE_${plan.toUpperCase()}_PRICE_ID or country-specific price id.` });
    const st = stripe();
