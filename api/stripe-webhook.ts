...
async function activatePlan(userId: string, plan: string, customerId?: string, subscriptionId?: string) {
  const sb = admin();
  if (!sb || !userId || !['Standard', 'Premium', 'Max'].includes(plan)) return;
...
async function markSubscription(userId: string, plan: string, status: string, subscriptionId?: string) {
  ...
  await sb.from('qv_subscriptions').upsert({ user_id: userId, plan: ['Standard','Premium','Max'].includes(plan) ? plan : 'Free', status: normalized, provider: 'stripe', provider_reference: subscriptionId || null, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
