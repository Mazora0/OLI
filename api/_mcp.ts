...
if (runningOnVercel) {
  ...
  for (const file of apiFiles) {
    ...
    const updated = original.replace(
      ...
      (_match, quote, helperName) =>
        `${quote}../src/server/_${helperName}${quote}`
    );
  ...
  console.log(
    'Vercel API helper modules moved out of /api for this build.'
  );
} else {
  console.log(
    'Server helper copies prepared for local build.'
  );
}
