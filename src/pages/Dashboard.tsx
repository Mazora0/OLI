...
import { BarChart3, BrainCircuit, CreditCard, Database, Lock, ShieldCheck, Sparkles, Zap } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { guestLimits, limits, price } from '../lib/pricing';
...
const copy = {
  ar: {
    badge: 'مركز التحكم', title: 'لوحة تحكم Qalvero', currentPlan: 'الخطة الحالية', upgrade: 'ترقية الخطة', daily: 'حدود الرسائل اليومية', used: 'مستخدم', left: 'متبقي', serverTracked: 'الأرقام الحقيقية بتتجاب من Supabase لما تكون مسجل دخول.', region: 'منطقة الدفع', models: 'موديلات متاحة', guest: 'ضيف', loginHint: 'سجّل دخول عشان تشوف استخدامك الحقيقي وحدود حسابك.', login: 'تسجيل الدخول', next: 'حالة الإنتاج', nextText: 'لو ظهر خطأ في الشات، راجع مفاتيح AI و Supabase في Vercel. الواجهة لا تستخدم أزرار وهمية.', cards: [ ['QLO Flash', 'ردود يومية سريعة بحدود حسب الخطة.', Zap], ['QLO Pro', '٤ رسائل يوميًا في الخطة المجانية بعد تسجيل الدخول.', BrainCircuit], ['ذاكرة بسيطة', 'تحفظ الاسم والدراسة والهدف من الشات للمسجلين.', Database], ['أمان الحساب', 'Supabase Auth + RLS + مفاتيح السيرفر.', ShieldCheck] ]
  },
  en: {
    badge: 'Control Center', title: 'Qalvero Dashboard', currentPlan: 'Current plan', upgrade: 'Upgrade plan', daily: 'Daily message limits', used: 'Used', left: 'Left', serverTracked: 'Real numbers are read from Supabase when you are signed in.', region: 'Billing region', models: 'Available models', guest: 'Guest', loginHint: 'Sign in to see real usage and account limits.', login: 'Login', next: 'Production status', nextText: 'If chat returns an error, check AI and Supabase keys in Vercel. The UI avoids fake buttons.', cards: [ ['QLO Flash', 'Fast daily replies with limits by plan.', Zap], ['QLO Pro', '4 daily Pro messages on the free plan after login.', BrainCircuit], ['Compact memory', 'Saves basics like name, study, and goal from chat.', Database], ['Account security', 'Supabase Auth + RLS + server keys.', ShieldCheck] ]
  }
} as const;
...
export default function Dashboard() {
  ...
  const usageCards = [
    { name: 'QLO Flash', used: flashUsed, limit: flashLimit },
    { name: 'QLO Pro', used: proUsed, limit: proLimit }
  ];
    ...
    <section className="mx-auto max-w-6xl">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 soft-text">{profile?.email || c.guest} · {c.currentPlan}: <b className="strong-muted">{isGuest ? c.guest : plan}</b></p>
        </div>
        {isGuest ? <Link to="/login" className="btn btn-primary"><Lock size={18} /> {c.login}</Link> : <Link to="/pricing" className="btn btn-primary"><CreditCard size={18} /> {c.upgrade}</Link>}
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="card rounded-[2rem] p-5"><p className="text-sm soft-text">{c.models}</p><h2 className="mt-2 text-4xl font-black">{isGuest ? 1 : limits[plan].models.length}</h2><p className="mt-2 text-sm soft-text">{isGuest ? 'QLO Flash' : limits[plan].models.join(', ')}</p></div>
      </div>
